...
Report_DoReport(Bank *bank, int workerNum)
{
  ...
  if(bank->numToFinish == 0){ //if i am the last
    bank->numToFinish = bank->workers;
    ...
    if (rpt->numReports >= MAX_NUM_REPORTS) {
      ...
      for(int i = 0; i < bank->workers - 1; i ++){ //free the ones who wait
    ...
    */
    ...
    sem_post(&bank->check);
    sem_wait(&bank->nextDay); //express the readiness for the next day
    sem_post(&bank->work);
  }
  return 0;
}
