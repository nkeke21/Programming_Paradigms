...
(if (null? (cdr lst)) '()
(append (apply append (map (lambda (x)  (intersect (car lst) x)) (cdr lst))) (intersection-points (cdr lst)))))

(define (distance-product elem lst) 
(apply * (map (lambda (x) (dist elem x)) (remove elem lst))))

(define (rate-points lst)
(map (lambda (x) (cons (distance-product x  lst)  (list x))) lst))
...
(define (halfls lst halflen)
  (if (zero? halflen) '()
  (cons (cadar lst) (halfls (cdr lst) (- halflen 1))))
)

(define (clumped-points lst) 
  (halfls (sort-points (rate-points lst)) (quotient (length lst) 2))      
)

(define (average-point lst)
  (let ((x (/ (apply + (map (lambda (x) (car x)) lst)) (length lst))) (y (/ (apply + (map (lambda (x) (cadr x)) lst)) (length lst))))
  (cons (distance-product (list x y) lst) (list (list x y))))
)
...
(define (where-am-i dst strs)
  (sort-points (map best-estimate (all-guesses dst strs)))
)
