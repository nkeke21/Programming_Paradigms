...
static void BuildIndices(const char *feedsFileName, hashset* sWords, hashset* h);
static void ProcessFeed(const char *remoteDocumentName, hashset* sWords, hashset* h);
static void PullAllNewsItems(FILE *dataStream, hashset* sWords, hashset* h);
static bool GetNextItemTag(streamtokenizer *st);
static void ProcessSingleNewsItem(streamtokenizer *st, hashset* sWords, hashset* h);
...
static void ParseArticle(const char *articleTitle,
                         ...
                         const char *articleURL, hashset* sWords, hashset* h);
static void ScanArticle(streamtokenizer *st, const char *articleTitle,
                        const char *unused, const char *articleURL, hashset* sWords, hashset* h);
static void QueryIndices(hashset* sWords, hashset* h);
static void fProcessResponse(const char *word, hashset* sWords, hashset* h);
static bool WordIsWellFormed(const char *word);

static void toLower(char* s);

static void fillHash(hashset* sWords);
...
static const int stop = 1009;
static const int sent = 10007;
static const int allocLen = 4;
static const int startInd = 0;

static const char* stopList = "data/stop-words.txt";
...
void freeElems(void* elem){
	free(((elems*)elem)->url);
	free(((elems*)elem)->article);
}
...
static int stopWordHash(const void* s, int numBuckets){
	return StringHash((char*)s, numBuckets);
}
static int vectHash(const void* s, int numBuckets){
  vect* v = (vect*)s;
  return StringHash(v->word,numBuckets);
}
static int vectCompare(const void* s1, const void* s2){
  return strcmp(((vect*)s1)->word,((vect*)s2)->word);
}
static int freqSort(const void* s1, const void* s2){
	elems* ll = (elems*)s1;
	elems* ss = (elems*)s2;
	if(ll->freq > ss->freq){
		return 1;
	} else if (ll->freq < ss->freq){
		return -1;
	}
	return 0;
}
...
static int compare(const void* s1, const void* s2){
	vect* ll = (vect*)s1;
	vect* ss = (vect*)s2;
	return strcmp((char*)ll->word, (char*)ss->word);
}

static int setCompare(const void* s1, const void* s2){
	elems* ll = (elems*)s1;
	elems* ss = (elems*)s2;
	return strcmp((char*)ll->url, (char*)ss->url);
}

static int stopWordCmp(const void* s1, const void* s2){
...
static void sorts(hashset* h){
	for(int i = 0; i < sent; i ++){
		vector* curr = &h->buckets[i];
		for(int j = 0; j < VectorLength(curr); j ++){
			vect* currElem = (vect*)VectorNth(curr, j);
			VectorSort(currElem->vec, freqSort);
		}
	}
}
...
int main(int argc, char **argv) {
  ...
  hashset sWords;
  HashSetNew(&sWords,  sizeof(char*), stop, stopWordHash, stopWordCmp, NULL);
  fillHash(&sWords);
  
  hashset h;
  HashSetNew(&h,  sizeof(vect), sent, vectHash, vectCompare, freeVect);
  
  BuildIndices((argc == 1) ? kDefaultFeedsFile : argv[1], &sWords, &h);
 // printf("shesvla\n");
  sorts(&h);
  QueryIndices(&sWords, &h);
  curl_global_cleanup();
  //printf("gasvla\n");
  HashSetDispose(&h);
  HashSetDispose(&sWords);
  return 0;
}
...
static void BuildIndices(const char *feedsFileName, hashset* sWords, hashset* h){
  ...
  while (STSkipUntil(&st, ":") !=
    ...
    ProcessFeed(remoteFileName, sWords, h);
  }
...
static void ProcessFeedFromFile(char *fileName, hashset* sWords, hashset* h) {
  FILE *infile;
  streamtokenizer st;
...
static void ProcessFeed(const char *remoteDocumentName, hashset* sWords, hashset* h){
  ...
  if (!strncmp(kFilePrefix, remoteDocumentName, strlen(kFilePrefix))) {
    ProcessFeedFromFile((char *)remoteDocumentName + strlen(kFilePrefix), sWords, h);
    return;
  }

  FILE *tmpFeed = FetchURL(remoteDocumentName, "tmp_feed");
  PullAllNewsItems(tmpFeed, sWords, h);
  fclose(tmpFeed);
}
...
static void PullAllNewsItems(FILE *dataStream, hashset* sWords, hashset* h){
  ...
  while (GetNextItemTag(
    ...
    ProcessSingleNewsItem(&st, sWords, h);
  }
...
static void ProcessSingleNewsItem(streamtokenizer *st, hashset* sWords, hashset* h){
  ...
  ParseArticle(articleTitle, articleDescription, articleURL, sWords, h);
}
...
static void ParseArticle(const char *articleTitle,
  ...
  ScanArticle(&st, articleTitle, articleDescription, articleURL, sWords, h);
  STDispose(&st);
  fclose(tmpDoc);
}
...
static void ScanArticle(streamtokenizer *st, const char *articleTitle,
 ...
 // char* articleTitle1 = strdup(articleTitle);
//  char* articleURL1 = strdup(articleTitle);
  ...
  while (STNextToken(st, word, sizeof(word))) {
    if (strcasecmp(word, "<") == 0) {
      ...
      if (WordIsWellFormed(word)) {
          ...
          if(HashSetLookup(sWords, dummy) == NULL){
          	vect newNode;
          	newNode.word = dummy;
          	int index = StringHash(newNode.word, sent);
          	//printf("shesvla\n");
          	int pos = VectorSearch(&h->buckets[index], &newNode,  compare, startInd, false);
          	//printf("gasvla\n");
            	if(pos == -1){
          		vector* helper = malloc(sizeof(vector));
          		VectorNew(helper, sizeof(elems), freeElems, allocLen);
          		newNode.vec = helper;
              		elems tmp;
              		tmp.freq = 1;
		        tmp.url = strdup(articleURL);
		        tmp.article = strdup(articleTitle);
             	 	VectorAppend(newNode.vec,&tmp);
              		HashSetEnter(h,&newNode);
          	} 
            	else{
              		vector* curr = &h->buckets[index];
              		vect* dest = VectorNth(curr, pos);
             		free(dummy);
              		elems nn;
              		nn.url= strdup(articleURL);
              		nn.article = strdup(articleTitle);
              		int pos = VectorSearch(dest->vec,&nn,setCompare,startInd,false);
              		if(pos==-1){
                		nn.freq = 1;
                		VectorAppend(dest->vec,&nn);
              		}
              		else{
                		free(nn.url);
                		elems* ptr = VectorNth(dest->vec,pos);
                		ptr->freq++;
              		}
            }
	}
...
static void QueryIndices(hashset* sWords, hashset* h){
  ...
  while (true) {
    ...
    if (strcasecmp(response, "") == 0)
      break;
    fProcessResponse(response, sWords, h);
...
static void fProcessResponse(const char *word, hashset* sWords, hashset* h){
  if (WordIsWellFormed(word)) {
   	char* ww = strdup(word);
   	toLower(ww);
   	if(HashSetLookup(sWords, ww) != NULL){
   		printf("Too common a word to be taken seriously. Try something more specific.");
   	} else {
   		int len = maxLen;
      		vect dest;
      		dest.word = ww;
      		vect* new = HashSetLookup(h,&dest);
      		if(new!=NULL){
      			vect* currElem = new;
   			if(VectorLength(currElem->vec) < len){
				len = VectorLength(currElem->vec);
			}
   			vector* helper = currElem->vec;
   			for(int i = len - 1; i >= 0; i --){
   				elems* currElem = VectorNth(helper, i);
   				printf("%d.) ", len - i);
   				printf("\"%s\" ", currElem->article);
   				if(currElem->freq == 1){
