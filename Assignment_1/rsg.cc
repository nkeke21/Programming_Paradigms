...
string Strings(map<string, Definition>& Grammar, string currKey); //Creating a string

void printing(map<string, Definition>& Grammar); //Printing strings
...
/*Forming a string, using recursion */
string Strings(map<string, Definition>& Grammar, string currKey){
	string result = "";
	Definition def = Grammar[currKey];
  	Production prod = def.getRandomProduction();
  	
  	for (Production::iterator curr = prod.begin(); curr != prod.end(); ++curr) {
  		if(curr[0][0] == '<'){
			result += Strings(Grammar, curr[0]);	
  		} else {
			result +=  " "+ curr[0];
		}
  	}
...
void printing(map<string, Definition>& Grammar){
	  for(int i = 0; i < 3; i ++){
		cout << "Version #" << i + 1 << ": --------------------------- " << endl;
  		cout << Strings(Grammar, "<start>") << endl;
  		cout << "" << endl;
  	}
}
