...
class imdb {
 ...
 private:
  ...
  /*
  * struct, actors, save information about the actorFile which we want to use 
  * in compfunc. Without this struct we would not have access to the actorFile.
  * We also have actor's name in this struct to compare it to other actors' names
  */
  struct actors{
	const void* actorFile;
  	string actor;
  ...
  /*
  * struct, movies, save information about the movieFile which we want to use 
  * in compfun. Without this struct we would not have access to the movieFile.
  * We also have information about the film to compare it to other actors' names.
  */
  struct movies{
	const void* movieFile;
	film films;
   ...
   /*
  * cmpfunc compare input actor's name to the names of actors in 
  * the array. This fuction helps us also in  binary search. 
  */
  static int cmpfunc(const void* info, const void* ptr);
  
  /*
  * cmpfunc compare input film's title to the titles of films in the array. 
  * This fuction helps us also in binary search. Because of the name of the films
  * sometimes are the same, we need something more, for example date of the film.
  * With year we can conclue if films with the same title are the same films. 
  */
  static int cmpfun(const void* infoM, const void* ptrM);
